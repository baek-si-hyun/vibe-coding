...
func buildQuantItem(symbol string, candles []Candle, minTradeValue24H float64) *QuantItem {
	if len(candles) < quantMinRequired {
		return nil
	}
	...
	for i, candle := range candles {
		...
		highs[i] = candle.High
		tradeValues[i] = candle.TradeValue
	...
	ema24 := ema(tailFloat(closes, quantCandles24H), 12)
	recentHigh := 0.0
	...
	breakout := 0.0
	if recentHigh > 0 {
		breakout = (price/recentHigh - 1) * 100
	}
	stretch := 0.0
	if ema24 > 0 {
		stretch = (price/ema24 - 1) * 100
	}

	rawMomentumShort := 0.35*return1h + 0.65*return24h
	rawTrend := 0.60*pctChange(price, ema24) + 0.40*return24h
	rawBreakout := 0.70*breakout + 0.30*return24h
	rawStability := -vol24h
	...
	return &QuantItem{
		...
		rawMomentumShort:   rawMomentumShort,
		rawLiquidityNow:    math.Log10(math.Max(value24h, 1)),
		rawLiquidityFlow:   valueRatio24H,
		rawStability:       rawStability,
		rawDrawdown:        rawDrawdown,
		rawTrend:           rawTrend,
		rawBreakout:        rawBreakout,
		rawStretch:         stretch,
	}
}
...
func candidateCount(limit int) int {
	count := limit * 2
	if count < quantCandidateMin {
		count = quantCandidateMin
	}
	if count > quantCandidateMax {
		count = quantCandidateMax
	}
	return count
}

func marketSymbol(market string) string {
	if symbol, ok := strings.CutPrefix(market, "KRW-"); ok {
		return symbol
	}
	return market
}
...
func (s *Service) GetQuantData(limit int, minTradeValue24H float64) (map[string]any, error) {
	...
	if minTradeValue24H <= 0 {
		minTradeValue24H = defaultMinTradeValue
	}
	...
	tickers, err := s.fetchTickersKRW()
	if err != nil {
		return nil, fmt.Errorf("업비트 티커 조회 실패: %w", err)
	}

	filtered := make([]tickerItem, 0, len(tickers))
	for _, ticker := range tickers {
		if ticker.AccTradePrice24H >= minTradeValue24H {
			filtered = append(filtered, ticker)
		}
	}
	if len(filtered) == 0 {
		return nil, fmt.Errorf("조건을 만족하는 업비트 KRW 코인이 없습니다")
	}

	if len(filtered) > candidateCount(limit) {
		filtered = filtered[:candidateCount(limit)]
	}

	items := make([]QuantItem, 0, len(filtered))
	sem := make(chan struct{}, quantConcurrency)
	results := make(chan *QuantItem, len(filtered))
	...
	for _, ticker := range filtered {
		...
		go func() {
			...
			sem <- struct{}{}
			defer func() { <-sem }()
			candles, fetchErr := s.fetchCandles(ticker.Market)
			if fetchErr != nil {
				return
			}
			if item := buildQuantItem(marketSymbol(ticker.Market), candles, minTradeValue24H); item != nil {
				results <- item
			}
		}()
	}
	...
	if len(items) == 0 {
		return nil, fmt.Errorf("조건을 만족하는 업비트 코인 랭킹을 계산하지 못했습니다")
	}
	...
	if len(items) == 0 {
		return nil, fmt.Errorf("신중한 기준을 만족하는 업비트 추천 코인이 없습니다")
	}
	for i := range items {
		items[i].Rank = i + 1
	}
