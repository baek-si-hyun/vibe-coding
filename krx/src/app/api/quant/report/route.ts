...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${BACKEND_BASE}/api/quant/report`);
    ...
    if (!res.ok) {
      if (res.status === 404) {
        return NextResponse.json(
          {
            error: `백엔드에 /api/quant/report 라우트가 없습니다 (${BACKEND_BASE}). backend-go 서버를 최신 코드로 재시작하세요.`,
    ...
    return NextResponse.json(
      {
        error: `퀀트 리포트 조회 중 오류가 발생했습니다. backend-go(${BACKEND_BASE})가 실행 중인지 확인하세요.`,
        message,
      },
      { status: 500 }
    );
